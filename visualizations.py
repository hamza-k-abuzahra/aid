...
# def animate(sim, frames=1000, interval=50):
#     # Precompute static data
#     initial_positions = np.array([uav.initial_pos for uav in sim.uavs])
#     # uav_ids = np.array(list(sim.uavs.keys()))
...
#     # Create static sensor positions
#     ax.scatter(
#         initial_positions[:, 0], 
#         initial_positions[:, 1], 
#         color='yellow', 
...
#         current_positions = np.array([uav.curr_pos for uav in sim.uavs])
#         uav_scatter.set_offsets(current_positions)
        
#         # Update UAV colors based on state
#         colors = np.array([
#             'purple' if uav.is_leader else 
#             'red' if uav.has_info else 
#             'blue' 
#             for uav in sim.uavs
#         ])
...
#         # Update paths
#         for i, uav in enumerate(sim.uavs):
#             if uav.path and i < len(path_lines):
#                 path_coords = np.array([sim.uavs[p].initial_pos for p in uav.path])
#                 path_lines[i].set_data(path_coords[:, 0], path_coords[:, 1])
